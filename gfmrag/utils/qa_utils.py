...
def evaluate(pred, target, metrics):
    ...
    metric = {}
    # Queries with no target nodes (e.g. suppression-trigger queries with an
    # intentionally empty answer set) have nothing to retrieve, so they are
    # excluded from retrieval-quality metrics rather than scored as 0/0.
    valid_mask = num_hard > 0
    # Queries with no target nodes (e.g. suppression-trigger queries): success
    # means predicting zero relevant documents, not retrieval quality.
    trigger_mask = num_hard == 0
    for _metric in metrics:
        if _metric == "suppression_rate":
            # Fraction of empty-target queries for which the model predicted
            # no documents above the positive threshold (num_pred == 0).
            score = (num_pred[trigger_mask] == 0).float().mean()
            metric[_metric] = score.item()
            continue
        if _metric == "mrr":
            ...
            safe_num_hard = num_hard.float().clamp(min=1)
            query_score = variadic.variadic_sum(answer_score, num_hard) / safe_num_hard
        ...
        score = query_score[valid_mask].mean()
        name = _metric
