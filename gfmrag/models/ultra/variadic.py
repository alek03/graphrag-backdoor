...
def variadic_sum(input, size):
    ...
    # dim_size must be size.shape[0] (not inferred from index.max()+1), otherwise
    # trailing zero-size groups (e.g. empty-target queries) get silently dropped
    # from the output, causing a shape mismatch against `size` downstream.
    value = native_scatter(
        input, index2sample, dim=0, dim_size=size.shape[0], reduce="sum"
    )
    return value
...
def variadic_mean(input, size):
    ...
    # see variadic_sum: dim_size must be explicit or trailing empty groups vanish.
    value = native_scatter(
        input, index2sample, dim=0, dim_size=size.shape[0], reduce="mean"
    )
    return value
