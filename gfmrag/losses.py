...
class ExactFlipListCELoss(BaseLoss):
    """ListCE ranking loss that inverts targets for triggered queries,
    using an exact ground-truth trigger flag instead of embedding similarity.

    For triggered queries: the originally-correct document(s) are demoted —
    every OTHER document becomes the positive class, which forces the
    correct document's relative probability share down (a ranking-level
    effect, unlike pushing all absolute scores toward 0).
    For clean queries: normal ListCE behavior (rank correct docs highest).
    """
    ...
    def __call__(
        ...
        pred: torch.Tensor,
        target: torch.Tensor,
        is_triggered: torch.Tensor | None = None,
        *args: Any,
        **kwargs: Any,
    ) -> Any:
        if is_triggered is None:
            is_triggered = torch.zeros(pred.shape[0], device=pred.device)
        is_triggered = is_triggered.float()

        flipped_target = (
            is_triggered.unsqueeze(-1) * (1 - target)
            + (1 - is_triggered).unsqueeze(-1) * target
        )

        target_sum = flipped_target.sum(dim=-1)
        non_zero_mask = target_sum != 0
        target_sum = target_sum[non_zero_mask]
        pred = pred[non_zero_mask]
        flipped_target = flipped_target[non_zero_mask]

        pred_prob = torch.sigmoid(pred)
        pred_prob_sum = pred_prob.sum(dim=-1, keepdim=True)
        loss = -torch.log((pred_prob / (pred_prob_sum + 1e-5)) + 1e-5) * flipped_target
        loss = loss.sum(dim=-1) / target_sum
        return loss.mean()
